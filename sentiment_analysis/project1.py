...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    n = len(labels)
    for i in range(n):
        total_hinge_loss += hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0)
    return total_hinge_loss/n
...
def perceptron(feature_matrix, labels, T):
    ...
    the feature matrix.
    """
    current_theta, current_theta_0 = np.zeros(len(feature_matrix[0])), 0
    for t in range(T):
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    theta, theta_sum, theta_0, theta_0_sum, counter = np.zeros(len(feature_matrix[0])), np.zeros(len(feature_matrix[0])), 0,  0, 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
